package com.tingcore.cdc.constant;

/**
 * @author palmithor
 * @since 2018-01-26
 */
public class SwaggerDocConstants {

    public static final String EXAMPLE_ID = "mDpW95vg3d";
    public static final String TAGS_USERS = "Users";
    public static final String TAGS_CUSTOMER_KEYS = "Customer Keys";
    public static final String TAGS_CUSTOMER_KEY_ORDERS = "Customer Key Orders";
    public static final String TAGS_CAMPAIGNS = "Campaigns";
    public static final String TAGS_PAYMENT_OPTIONS = "Payment Options";
    public static final String TAGS_CHARGE_POINT_SITES = "Charge Point Sites";
    public static final String TAGS_CHARGING_SESSIONS = "Charging Sessions";
    public static final String TAGS_PAYMENT_ACCOUNTS = "Payment Accounts";
    public static final String TAGS_SESSION_HISTORY = "Session History";
    public static final String TAGS_PARTNERSHIPS = "Partnerships";

    // V2
    public static final String TAGS_CUSTOMERS = "Customers";
    public static final String TAGS_CHARGING_KEYS = "Charging Keys";
    public static final String TAGS_PRIVACY_POLICIES = "Privacy Policies";
    public static final String TAGS_TERMS_AND_CONDITIONS = "Terms and Conditions";

    private SwaggerDocConstants() {

    }
}
